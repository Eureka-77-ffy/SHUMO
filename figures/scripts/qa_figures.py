...
def main():
    ...
    for stem in STEMS:
        ...
        if "svg" in paths:
            ...
            if stem in MATPLOTLIB_STEMS:
                ...
                assert "共享内核" in svg_text and "结构假设" in svg_text, stem
        else:
    ...
    assert all(term in roadmap_text for term in required_roadmap_terms)
    ...
    manifest["qa"] = {
        ...
        "roadmap_self_score": {"text_readability": 9, "arrow_accuracy": 10, "color_consistency": 10, "layout_consistency": 10, "style_fit": 9, "total": 48},
